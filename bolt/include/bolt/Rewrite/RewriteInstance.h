...
private:
  ...
  void handleRelocation(BinarySection &RelocatedSection,
  ...
  // Get sections within address space of a provided segment
  std::vector<BinarySection *> getSectionsForSegment(const ProgramHeader &Phdr,
  ...
  // Copy non-loadable segments to the output, updating offsets/addresses
  void mapNonLoadableSegments();
...
#define ELF_FUNCTION(TYPE, FUNC)                                               \
  ...
  /// part of .bss section.
  uint64_t getFileOffsetForAddress(const uint64_t Address) const;
